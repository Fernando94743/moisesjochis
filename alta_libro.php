...
// EL CADENERO: Si no hay sesión, lo pateamos al login
if (!isset($_SESSION['usuario'])) {
    header("Location: index.php"); // Cambia index.php por tu archivo de login real
    exit();
}
...
$pdo = conectarDB();
// Corregido: Usamos SELECT para mostrar la tabla, no DELETE.
$stmt = $pdo->query("SELECT * FROM libros ORDER BY id_libro DESC");
                            ...
                            <tbody>
                                <?php if (count($libros) > 0): ?>
                                    <?php foreach ($libros as $libro): ?>
                                        <tr>
                                            <td class="ps-4 fw-bold"><?php echo htmlspecialchars($libro['id_libro']); ?></td>
                                            ...
                                            <td class="text-center pe-4">
                                                <a href="eliminar_libro.php?id=<?php echo $libro['id_libro']; ?>" class="btn btn-delete shadow-sm" title="Eliminar" onclick="return confirm('¿Estás seguro de eliminar este libro?');">
